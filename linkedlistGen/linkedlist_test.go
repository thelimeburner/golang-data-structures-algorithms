...
func TestAdd(t *testing.T) {
	...
	table2 := []struct {
		...
		{[]string{"ho", "hi", "ha", "eh"}, []string{"ho", "hi", "ha", "eh"}},
...
func TestDel(t *testing.T) {
	...
	table_string := []struct {
		...
		{[]string{"hi", "ho", "hi", "ho", "hi"}, []string{"hi", "ho"}, []string{"hi", "ho", "hi"}},
		{[]string{"Hello", "mad", "World!"}, []string{"mad"}, []string{"Hello", "World!"}},
		// {[]int{}, []int{}},
	}

	for _, tc := range table_string {
		list1 := New()
		//fill up the list
		for _, val := range tc.vals {
			list1.Append(val)
